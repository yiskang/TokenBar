...
| Rust FFI | `crates/tb_core_ffi/` | Stable C-callable entry points, report options, panic boundary, JSON serialization, and static-library linkage | A second Swift-side parser or an implicit UI filter |
...
| Pace projections | Swift `TokenBarCore` | Treat as a presentation calculation over quota windows, not as a replacement for provider truth |
...
| Does the change cross Rust, C, and Swift? | `ctb.h`, FFI mapper, Swift decoder, and parity test or smoke evidence |
