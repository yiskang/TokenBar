...
| Evaluator baseline | Store cadence 對齊 CodexBar `4abfbb6c`；evaluator 固定對齊 `f986661480c7862bc42b09ad37e5dc781a7353d3` | 避免只用 worked cases 卻移植出另一套合理公式 |
...
| Expected curve | 每格取 recency-weighted median，再算 `lambda * historical + (1 - lambda) * linear`；依 pinned revision 將結果 clamp 到 `0...linearBaseline`，最後 monotonicize |
...
| Evaluator parity | Pinned CodexBar revisions 的 reset jitter、early exhaustion、current actual shift、risk smoothing 與 100% cases |
